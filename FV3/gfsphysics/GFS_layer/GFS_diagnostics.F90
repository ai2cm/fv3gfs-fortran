...
!--- three-dimensional variables that need to be handled special when writing 
    ...
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_tendency_of_specific_humidity_due_to_turbulence'
    ExtDiag(idx)%desc = 'vertically integrated instantaneous water vapor tendency due to turbulence scheme'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    ...
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_tendency_of_specific_humidity_due_to_deep_convection'
    ExtDiag(idx)%desc = 'vertically integrated instantaneous water vapor tendency due to deep convection'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    ...
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_tendency_of_specific_humidity_due_to_shallow_convection'
    ExtDiag(idx)%desc = 'vertically integrated instantaneous water vapor tendency due to shallow convection'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    ...
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_tendency_of_specific_humidity_due_to_microphysics'
    ExtDiag(idx)%desc = 'vertically integrated instantaneous water vapor tendency due to microphysics'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    ...
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_tendency_of_specific_humidity_due_to_change_in_atmosphere_mass'
    ExtDiag(idx)%desc = 'vertically integrated instantaneous residual water vapor tendency'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    ...
    ! Time average physics component tendency diagnostics
    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_longwave_heating'
    ExtDiag(idx)%desc = 'average temperature tendency due to longwave radiation'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,1)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_shortwave_heating'
    ExtDiag(idx)%desc = 'average temperature tendency due to shortwave radiation'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,2)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_turbulence'
    ExtDiag(idx)%desc = 'average temperature tendency due to turbulence scheme'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,3)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_deep_convection'
    ExtDiag(idx)%desc = 'average temperature tendency due to deep convection'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,4)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_shallow_convection'
    ExtDiag(idx)%desc = 'average temperature tendency due to shallow convection'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,5)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_microphysics'
    ExtDiag(idx)%desc = 'average temperature tendency due to micro-physics'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,6)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_dissipation_of_gravity_waves'
    ExtDiag(idx)%desc = 'average temperature tendency due to gravity wave drag'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,7)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_longwave_heating_assuming_clear_sky'
    ExtDiag(idx)%desc = 'average temperature tendency due to clear sky longwave radiation'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,8)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_air_temperature_due_to_shortwave_heating_assuming_clear_sky'
    ExtDiag(idx)%desc = 'average temperature tendency due to clear sky shortwave radiation'
    ExtDiag(idx)%unit = 'K/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%t_dt_average(:,:,9)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_specific_humidity_due_to_turbulence'
    ExtDiag(idx)%desc = 'average water vapor tendency due to turbulence scheme'
    ExtDiag(idx)%unit = 'kg/kg/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%q_dt_average(:,:,1)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_specific_humidity_due_to_deep_convection'
    ExtDiag(idx)%desc = 'average water vapor tendency due to deep convection'
    ExtDiag(idx)%unit = 'kg/kg/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%q_dt_average(:,:,2)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_specific_humidity_due_to_shallow_convection'
    ExtDiag(idx)%desc = 'average water vapor tendency due to shallow convection'
    ExtDiag(idx)%unit = 'kg/kg/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%q_dt_average(:,:,3)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_specific_humidity_due_to_microphysics'
    ExtDiag(idx)%desc = 'average water vapor tendency due to microphysics'
    ExtDiag(idx)%unit = 'kg/kg/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%q_dt_average(:,:,4)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 3
    ExtDiag(idx)%name = 'average_tendency_of_specific_humidity_due_to_change_in_atmosphere_mass'
    ExtDiag(idx)%desc = 'average residual water vapor tendency'
    ExtDiag(idx)%unit = 'kg/kg/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'mass_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var3 => IntDiag(nb)%q_dt_average(:,:,5)
    enddo

    ! Vertically integrated time average temperature tendency diagnostics
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_longwave_heating'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to longwave radiation'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,1)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_shortwave_heating'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to shortwave radiation'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,2)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_turbulence'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to turbulence scheme'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,3)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_deep_convection'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to deep convection'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%time_avg = .true.
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,4)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_shallow_convection'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to shallow convection'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,5)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_microphysics'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to micro-physics'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,6)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_dissipation_of_gravity_waves'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to gravity wave drag'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,7)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_longwave_heating_assuming_clear_sky'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to clear sky longwave radiation'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,8)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_air_temperature_due_to_shortwave_heating_assuming_clear_sky'
    ExtDiag(idx)%desc = 'vertically integrated average temperature tendency due to clear sky shortwave radiation'
    ExtDiag(idx)%unit = 'W/m**2'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%t_dt_average_int(:,9)
    enddo

! Vertically integrated average specific humidity tendency diagnostics
    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_specific_humidity_due_to_turbulence'
    ExtDiag(idx)%desc = 'vertically integrated average water vapor tendency due to turbulence scheme'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%q_dt_average_int(:,1)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_specific_humidity_due_to_deep_convection'
    ExtDiag(idx)%desc = 'vertically integrated average water vapor tendency due to deep convection'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%q_dt_average_int(:,2)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_specific_humidity_due_to_shallow_convection'
    ExtDiag(idx)%desc = 'vertically integrated average water vapor tendency due to shallow convection'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%q_dt_average_int(:,3)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_specific_humidity_due_to_microphysics'
    ExtDiag(idx)%desc = 'vertically integrated average water vapor tendency due to microphysics'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%q_dt_average_int(:,4)
    enddo

    idx = idx + 1
    ExtDiag(idx)%axes = 2
    ExtDiag(idx)%name = 'int_average_tendency_of_specific_humidity_due_to_change_in_atmosphere_mass'
    ExtDiag(idx)%desc = 'vertically integrated average residual water vapor tendency'
    ExtDiag(idx)%unit = 'kg/m**2/s'
    ExtDiag(idx)%mod_name = 'gfs_phys'
    ExtDiag(idx)%coarse_graining_method = 'area_weighted'
    ExtDiag(idx)%time_avg = .true.
    allocate (ExtDiag(idx)%data(nblks))
    do nb = 1,nblks
      ExtDiag(idx)%data(nb)%var2 => IntDiag(nb)%q_dt_average_int(:,5)
    enddo
!rab
