...
!--- IPD typdefs
  ...
  use coarse_graining_mod, only: vertical_remapping_requirements, vertically_remap_field
  use coarse_graining_mod, only: BLENDED_AREA_WEIGHTED, blended_area_weighted_coarse_grain_field
  !
      ...
      if (requested .and. Diag(index)%diag_manager_controlled) then
        ...
        elseif (Diag(index)%axes .eq. 3) then
          ...
          if (Diag_coarse(index)%id > 0) then
            ...
            elseif (&
              trim(coarsening_strategy) .eq. PRESSURE_LEVEL .or. &
              trim(coarsening_strategy) .eq. PRESSURE_LEVEL_EXTRAPOLATE) then
            ...
            elseif (trim(coarsening_strategy) .eq. BLENDED_AREA_WEIGHTED) then
              call store_data3D_coarse_blended_area_weighted(Diag_coarse(index)%id, Diag_coarse(index)%name, &
                Diag_coarse(index)%coarse_graining_method, &
