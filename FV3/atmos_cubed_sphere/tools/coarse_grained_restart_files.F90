module coarse_grained_restart_files_mod
  ...
  use coarse_graining_mod, only: compute_mass_weights, get_coarse_array_bounds,&
       get_fine_array_bounds, weighted_block_average, weighted_block_edge_average_x, weighted_block_edge_average_y, &
       mask_area_weights, block_upsample, remap_edges_along_x, &
       remap_edges_along_y, vertically_remap_field, block_min
...
contains
  ...
  subroutine coarse_grain_restart_data(Atm)
    ...
    elseif (trim(Atm%coarse_graining%strategy) .eq. PRESSURE_LEVEL_EXTRAPOLATE) then
       ...
       write(error_message, *) 'Currently only model_level, pressure_level, and blended_area_weighted &
