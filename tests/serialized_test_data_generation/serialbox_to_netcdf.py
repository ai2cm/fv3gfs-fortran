...
def main(data_path: str, output_path: str):
    ...
    for savepoint_name in sorted(list(savepoint_names)):
        ...
        serializer = get_serializer(data_path, rank=0)
        names_list = list(serializer.fields_at_savepoint(serializer.get_savepoint(savepoint_name)[0]))
        serializer_list = []
        ...
        if n_savepoints > 0:
            for varname in set(names_list).difference(["rank"]):
                ...
                data_vars[varname] = get_data(data_shape, total_ranks, n_savepoints, rank_list, varname)
