...
for exp_file in ${EXPERIMENTS} ; do
  ...
  if [ "${VALIDATE_ONLY}" == "true" ] ; then
      EXPERIMENT=${exp_name} DOCKER_RUN_ARGS=${DOCKER_RUN_ARGS} make generate_data validate_data
  else
      EXPERIMENT=${exp_name} DOCKER_RUN_ARGS=${DOCKER_RUN_ARGS} make distclean generate_data pack_data push_data distclean
  fi
  make distclean
  echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++"
  echo ""
